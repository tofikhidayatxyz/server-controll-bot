...
bot.on('message', (msg) => {
  ...
  if (msg.text.substring(0,3).toLowerCase() == "run") {
	...
	// git
	if (msg.text.substring(0,3).toLowerCase() == "git") { 
		let tmp =  msg.text.split("@");
		let dir  =  tmp[1].split(" ").join("")
...
/*
const user = 'tofikhidayat';
const pass = 'mazenda9999';
const dir = path.join(__dirname,"../smk");
const repo =  "github.com/tofikhidayat/smk";
const remote  =  `https://${user}:${pass}@${repo}`;

git(dir).pull(remote, "master", "--force" ).then((err)=> {
	console.log(err);
}).catch((e)=>{
	console.lcog(e)
})*/
